...
####
locals {
  accepter_aws_rtb_count  	    = "${length(data.aws_route_tables.peer_vpc_rts.ids)}"
}
...
resource "aws_route" "this_routes_region" {
  ...
  vpc_peering_connection_id = aws_vpc_peering_connection.this.id
  depends_on                = ["aws_vpc_peering_connection.this", "aws_vpc_peering_connection_accepter.peer_accepter"]
}
...
resource "aws_route" "peer_routes_region" {
  ...
  #count                     = length(var.peer_all_route_table_ids)
  route_table_id            = tolist(data.aws_route_tables.peer_vpc_rts.ids)[count.index]
  #route_table_id            = var.peer_all_route_table_ids[count.index]
  #route_table_id            = element(var.peer_all_route_table_ids, count.index)
